...
namespace NeuralNetwork.TurnBasedBoardGameTrainerStuff
{
        ...
        where TSquare : IGridSquare<TState>
    {
        ...
        public NeuralNet GetNet(IGridBoard<TState, TSquare> rootState, Players[] allActivePlayers, Func<BoardNetPair<TState, TSquare>, Random, bool> makeNeuralNetMove, Func<IGridBoard<TState, TSquare>, Random, bool>[] opponentMoves, int numberOfSimulations, int numberOfGenerations, Random random, NeuralNet preTrainedNet)
        {
            ...
            for (int i = 0; i < numberOfGenerations; i++)
            {
                ...
                best = Train(pairs, opponentMoveMap, neuralNetPlayer, makeNeuralNetMove, currentGeneration: i, random, preservePercent: 10, randomizePercent: 10, mutationMin: 0.5f, mutationMax: 1.5f, randomizeMin: -1, randomizeMax: 1);
            }
            return best;
        }
        ...
        private NeuralNet Train(List<BoardNetPair<TState, TSquare>> pairs, Dictionary<Players, Func<IGridBoard<TState, TSquare>, Random, bool>> opponentMoveMap, Players neuralNetPlayer, Func<BoardNetPair<TState, TSquare>, Random, bool> makeNeuralNetMove/*, Action<TSquare[][], Random>[] makeOpponentMove*/, int currentGeneration, Random random, double preservePercent, double randomizePercent, double mutationMin, double mutationMax, double randomizeMin, double randomizeMax)
        ...
        //Train only changes the nets of the pairs it doesn't reset the boards. Boards must be reset before Train function is called
        {
            for (int i = 0; i < pairs.Count; i++)
            {
                ...
                while (shouldContinue)
                {
                    ...
                    {
                        shouldContinue = makeNeuralNetMove(pairs[i], random);
                    }
                    else
                    {
